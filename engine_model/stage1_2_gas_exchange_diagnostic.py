"""
Stage 1.2 diagnostic: add simple intake/exhaust gas exchange (diagnostic-only)

Copies the Stage 1.2 NA baseline logic but introduces intake and exhaust
Reservoirs and Valve objects (Cantera) using the provisional Phase-0 valve
angles. This file is created as a diagnostic artifact only and does NOT
modify the original Stage 1.2 baseline or any Phase-0 CSV files.

Outputs diagnostic PNGs and prints a comparison against the closed-cycle
baseline values that were observed previously.
"""
...
# valve coefficients - reuse values consistent with the Cantera example
...
def run_gas_exchange(step_deg: float):
    ...
    print(f"\nRunning gas-exchange diagnostic with STEP_DEG = {step_deg} degrees")
    ...
    exhaust_open_deg = 540.0 - PHASE0['exhaust_open_deg_BBDC']
    exhaust_close_deg = 720.0 + PHASE0['exhaust_close_deg_TDC']
    ...
    for cycle in range(N_CYCLES):
        ...
        # create passive remote buffer reactor for Cantera Wall piston
        try:
            gas_remote = ct.Solution(MECH + '.yaml')
        except Exception:
        ...
        try:
            inlet = ct.Reservoir(gas_inlet, clone=False)
        except TypeError:
        ...
        # define outlet reservoir (ambient/exhaust)
        try:
        ...
        try:
            outlet = ct.Reservoir(gas_outlet, clone=False)
        except TypeError:
        ...
        # Reactor network with cylinder and remote buffer reactor
        sim = ct.ReactorNet([reactor, remote])
        ...
        for s in range(1, steps_per_cycle + 1):
            ...
            if (s > 1) and ((s - 1) * step_deg < theta_ignition <= s * step_deg):
                try:
                    ...
                    # re-create wall piston
                    piston = ct.Wall(reactor, remote)
                    piston.area = piston_area

                    # re-create valves attached to the new reactor so gas exchange continues
                    inlet = ct.Reservoir(gas_inlet, clone=False) if hasattr(ct.Reservoir, '__init__') else ct.Reservoir(gas_inlet)
                    inlet_valve = ct.Valve(inlet, reactor)
                    inlet_valve.valve_coeff = INLET_VALVE_COEFF * PHASE0['valve_effective_flow']
                    inlet_valve.time_function = (lambda t, open_r=inlet_open_rad, delta=inlet_delta: ((crank_angle(t) - open_r) % (4.0 * pi)) < delta)

                    outlet = ct.Reservoir(gas_outlet, clone=False) if hasattr(ct.Reservoir, '__init__') else ct.Reservoir(gas_outlet)
                    outlet_valve = ct.Valve(reactor, outlet)
                    outlet_valve.valve_coeff = OUTLET_VALVE_COEFF * PHASE0['valve_effective_flow']
                    outlet_valve.time_function = (lambda t, open_r=outlet_open_rad, delta=outlet_delta: ((crank_angle(t) - open_r) % (4.0 * pi)) < delta)
...
if __name__ == '__main__':
    ...
    print('Stage 1.2 gas-exchange diagnostic (diagnostic-only)')
    ...
    if pressures:
        ...
        plt.title('Stage1.2 Gas-Exchange: Pressure vs crank angle (final cycle)')
        plt.grid(True)
        plt.savefig(r'engine_model\stage1_2_gas_exchange_pressure.png')
        print('Saved pressure plot to engine_model\\stage1_2_gas_exchange_pressure.png')
    ...
    if pressures and volumes:
        ...
        plt.ylabel('pressure (bar)')
        plt.title('Stage1.2 Gas-Exchange: P-V loop (final cycle)')
        plt.grid(True)
        plt.savefig(r'engine_model\stage1_2_gas_exchange_pv.png')
        print('Saved PV plot to engine_model\\stage1_2_gas_exchange_pv.png')

    # print summary table
    def print_res(r):
        print(f"\nDiagnostic results for STEP_DEG = {r['step_deg']} deg:")
    ...
    # Compare against baseline closed-cycle values provided in the project summary
    baseline = {
        'work_J_per_cycle': -0.473574,
        'expansion_work_J': 68.5028,
        'compression_work_J': -68.9764,
        'max_pressure_bar': 1.499,
        'max_temperature_K': 3086.41,
    }

    print('\nComparison to closed-cycle baseline (diagnostic-only):')
    pct_work = (res['avg_work_J'] - baseline['work_J_per_cycle']) / abs(baseline['work_J_per_cycle']) * 100.0 if baseline['work_J_per_cycle'] != 0 else float('inf')
    print(f"  baseline avg work = {baseline['work_J_per_cycle']:.6g} J/cycle")
    print(f"  diagnostic avg work = {res['avg_work_J']:.6g} J/cycle  ({pct_work:+.2f}% change)")

    # expansion/compression from final cycle
    final = res['cycle_results'][-1]
    print(f"  diagnostic expansion = {final['expansion_work_J']:.6g} J, compression = {final['compression_work_J']:.6g} J")

    # whether net work became positive
    if res['avg_work_J'] > 0:
        print('\nRESULT: Net work became POSITIVE under this simple gas-exchange model (diagnostic-only).')
    else:
        print('\nRESULT: Net work remains NON-POSITIVE (<=0). Gas exchange alone did not resolve the negative-work result in this diagnostic.')

    # report mass flow diagnostics
    print('\nMass flow summary (final cycle):')
    print(f"  total intake mass passed (approx) = {final.get('total_mdot_in_kg', 0.0):.6g} kg")
    print(f"  total exhaust mass passed (approx) = {final.get('total_mdot_out_kg', 0.0):.6g} kg")

    print('\nDiagnostic file created: engine_model\\stage1_2_gas_exchange_diagnostic.py')
    print('Confirmation: original Stage 1.2 file was NOT modified.')

    # Do not mark Stage 1.2 complete; this is diagnostic-only
    print('\nFINAL STATUS: DIAGNOSTIC ONLY — DO NOT MARK STAGE 1.2 COMPLETE')
