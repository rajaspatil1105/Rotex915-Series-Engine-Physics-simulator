"""
Stage 1.2 diagnostic control: original Python-macro valve diagnostic with
valve_coeff = 1e-3 and outlet pressure = ambient (101325 Pa) to reproduce
previous near-zero flow result for comparison.

This file is a diagnostic copy. It does NOT modify the original Stage 1.2
baseline or any Phase-0 CSV files.
"""
...
# valve coefficients - set to 1e-3 for this control test
...
def run_gas_exchange(step_deg: float):
    ...
    print(f"\nRunning gas-exchange diagnostic control with STEP_DEG = {step_deg} degrees")
    ...
    for cycle in range(N_CYCLES):
        ...
        # For this control run keep outlet at ambient (no diagnostic lowering)
...
if __name__ == '__main__':
    ...
    print('Stage 1.2 gas-exchange diagnostic control (diagnostic-only)')
    ...
    if pressures:
        ...
        plt.title('Stage1.2 Gas-Exchange Control: Pressure vs crank angle (final cycle)')
        plt.grid(True)
        plt.savefig(r'engine_model\\stage1_2_gas_exchange_control_pressure.png')
        print('Saved pressure plot to engine_model\\stage1_2_gas_exchange_control_pressure.png')
    ...
    if pressures and volumes:
        ...
        plt.ylabel('pressure (bar)')
        plt.title('Stage1.2 Gas-Exchange Control: P-V loop (final cycle)')
        plt.grid(True)
        plt.savefig(r'engine_model\\stage1_2_gas_exchange_control_pv.png')
        print('Saved PV plot to engine_model\\stage1_2_gas_exchange_control_pv.png')