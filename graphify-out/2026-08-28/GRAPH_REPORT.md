...
## Corpus Check
- 33 files · ~49,854 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 332 nodes · 620 edges · 21 communities (18 shown, 3 thin omitted)
...
1. `Stage13HybridEngineOutput` - 38 edges
2. `RotaxReferenceModel` - 28 edges
3. `get_engine_state()` - 15 edges
4. `get_health_state()` - 15 edges
5. `_advance_case()` - 14 edges
6. `main()` - 14 edges
7. `run_stage1_3_baseline()` - 13 edges
8. `main()` - 12 edges
9. `torque_nm_from_power_kw()` - 10 edges
10. `HealthState` - 9 edges
...
- `run_gas_exchange()` --calls--> `crank_angle()`  [INFERRED]
  engine_model/stage1_2_gas_exchange_diagnostic.py → cantera_examples/ic_engine.py
- `test_boundary_extrapolation()` --calls--> `RotaxReferenceModel`  [EXTRACTED]
  test_rotax_reference_model.py → rotax_reference_model_v2.py
...
Cohesion: 0.10
Nodes (40): EngineState, _clamp(), egt_spread_limit_from_fuel_flow(), _entry(), generator_switch_state(), GeneratorSwitchState, get_health_state(), HealthState (+32 more)

### Community 4 - "Stage13HybridEngineOutput"
Cohesion: 0.08
Nodes (23): EngineOutputResult, isa_temp_c(), main(), DataFrame, ndarray, Path, Read-only hybrid output layer for Stage 1.3. Absolute outputs come from the…, Deterministic ambient surrogate for 3-input production queries. The CSV remains… (+15 more)
...
## Knowledge Gaps
- **3 isolated node(s):** `graphify`, `Rotax 915 iS — Engine Parameter Reference (Phase 1 — Step 0.2)`, `Rotax 915 iS — Missing / Additional Parameters (Phase 1 — Step 0.2)`
...
- **Why does `Stage13HybridEngineOutput` connect `Stage13HybridEngineOutput` to `stage4_health_validation.py`, `stage2_engine_state_smoke_validation.py`?**
...
- **Why does `torque_nm_from_power_kw()` connect `stage2_engine_state_smoke_validation.py` to `stage4_health_validation.py`, `Stage13HybridEngineOutput`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Are the 11 inferred relationships involving `Stage13HybridEngineOutput` (e.g. with `_validate_boost_sanity()` and `_validate_exact_csv_reproduction()`) actually correct?**
  _`Stage13HybridEngineOutput` has 11 INFERRED edges - model-reasoned connections that need verification._
- **What connects `graphify`, `Rotax 915 iS — Engine Parameter Reference (Phase 1 — Step 0.2)`, `Rotax 915 iS — Missing / Additional Parameters (Phase 1 — Step 0.2)` to the rest of the system?**
  _3 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `stage1_3_fuel_metered_combustion.py` be split into smaller, more focused modules?**
  _Cohesion score 0.13963963963963963 - nodes in this community are weakly interconnected._
- **Should `crank_angle` be split into smaller, more focused modules?**
